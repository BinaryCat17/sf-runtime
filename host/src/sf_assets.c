...
bool sf_loader_load_image(sf_engine* engine, const char* name, const char* path) {
    ...
    if (!sf_engine_resize_resource(engine, name, sh, n)) { stbi_image_free(data); return false; }
    ...
    else if (t->info.dtype == SF_DTYPE_U8) memcpy(t->buffer->data, data, p);
...
bool sf_loader_load_font(sf_engine* engine, const char* name, const char* path, float size) {
    ...
    int32_t sh[] = { ah * aw }; 
    if (sf_engine_resize_resource(engine, name, sh, 1)) {
        sf_tensor* t = sf_engine_map_resource(engine, name); 
        for(size_t i=0; i<(size_t)aw*ah; ++i) ((f32*)t->buffer->data)[i] = (f32)a[i] / 255.0f;
        sf_engine_sync_resource(engine, name);
    }
    ...
    if (sf_engine_resize_resource(engine, in, ish, 1)) {
        ...
        memcpy(ti->buffer->data, inf, mcp * 8 * sizeof(f32));
